...
const embedded_firecracker = @import("actiond_embedded_firecracker");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len > 1 and std.mem.eql(u8, args[1], actiond.firecracker_vm.fexec_argument)) {
        try actiond.firecracker_vm.fexecEmbedded(
            io,
            arena,
            init.minimal.environ,
            embedded_firecracker.executable,
            embedded_assets,
            args[2..],
        );
    }
    ...
    if (args.len > 1 and std.mem.eql(u8, args[1], "serve-vm")) {
        ...
        return actiond.linux_vm_host.serve(io, std.heap.smp_allocator, options);
    }
