...
if [[ ! -r /dev/kvm || ! -w /dev/kvm ]]; then
  echo "Linux Firecracker LLVM smoke requires read/write access to /dev/kvm" >&2
  exit 1
fi
if [[ "$#" -gt 1 ]]; then
...
setup_server() {
  ...
  if ! (
    cd "${repo_root}"
    bazel build --config=remote -c opt --remote_timeout=900 "${headers[@]}" "${server_target}"
  ) >"${build_log}" 2>&1; then
  ...
  if ! server="$({
    cd "${repo_root}"
    bazel cquery --config=remote -c opt --output=files --bes_backend= --noshow_progress "${headers[@]}" "${server_target}"
...
# Linux Firecracker LLVM Performance Comparison (${architecture})
...
Both measurements use fresh Bazel output bases on the same Linux host. The actiond measurement runs first; Firecracker is stopped before the Linux-host measurement.
EOF
